...
export async function removeActivation(profileName: string, target: "global" | "project", projectPath?: string): Promise<Activation | null> {
  const state = await readState();
  const idx = state.activations.findIndex(
    (a) => a.profile === profileName && a.target === target && a.projectPath === projectPath,
  );
...
export function getActivation(state: State, profileName: string): Activation | undefined {
