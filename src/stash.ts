...
export async function restoreOriginal(stashPath: string, originalPath: string): Promise<void> {
...
export async function stashSkillConfig(skillDir: string, skillName: string): Promise<void> {
...
export async function restoreSkillConfig(skillDir: string, skillName: string): Promise<void> {
  const stashPath = path.join(paths.configStashDir, skillName, "config.yaml");
  try {
