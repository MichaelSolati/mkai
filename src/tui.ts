...
import type { Conflict, Profile, Target } from "./types.js";
...
async function tuiActivate(): Promise<void> {
  ...
  const isActiveHere = (name: string) => state.activations.some((a) =>
    a.profile === name &&
  ...
  for (const name of selected) {
    const alreadyActiveHere = state.activations.some((a) =>
      a.profile === name &&
      a.target === target.kind &&
      (target.kind === "global" || a.projectPath === target.projectPath)
    );
...
async function tuiDeactivate(): Promise<void> {
  const state = await readState();
  const cwd = process.cwd();
  const activations = state.activations.filter((a) => a.target === "global" || a.projectPath === cwd);
  ...
  const options = [
    ...
    { value: "__all__", label: "Deactivate all shown", hint: `${activations.length} profiles` },
  ];

  const selected = await p.select({ message: "Which profile to deactivate?", options });
  if (p.isCancel(selected)) return;
  ...
  if (selected === "__all__") {
    for (const activation of activations) {
      ...
      p.log.success(`Deactivated "${activation.profile}" - ${result.linksRemoved} links removed, ${result.restored} originals restored.`);
    }
  } else {
    ...
    p.log.success(`Deactivated "${selected}" - ${result.linksRemoved} links removed, ${result.restored} originals restored.`);
  }
}
...
async function tuiCheck(): Promise<void> {
  ...
  for (const activation of state.activations) {
    for (const link of activation.links) {
      ...
      if (!healthy) {
        p.log.warn(`${chalk.red("broken")} ${activation.profile}/${link.name} → ${link.destination}`);
        broken++;
      }
    }
  }
