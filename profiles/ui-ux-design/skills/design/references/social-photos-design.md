...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-search`, `brand`, and `design-system` skills.
...
3. **Randomly invoke ONE of:** `/ck:ui-ux-search` OR `/ck:frontend-design` - for layout, hierarchy, visual balance. Pick one at random each run for design variety.
