...
Matches are stored at `Dating/<Name>/`. When a name collides, follow these rules.

### At new-match intake (Operation 1)

Before writing any files, run `obsidian list Dating/` and check whether a folder whose name starts with `<First>` already exists.

1. **No existing folder** → proceed normally, no suffix needed. `Dating/Sarah/`
2. **Exact folder match** (e.g. `Dating/Sarah/` exists and you're adding another Sarah):
...
1. Run `obsidian list Dating/` and collect all folders whose name starts with `<First>` (case-insensitive).
...
3. Build `profile.md` from `references/profile-template.md` with parsed details → `obsidian write "Dating/<Name>/profile.md" --file $TMPDIR/profile.md`
4. Build `conversation.md` skeleton from `references/conversation-template.md` with Pre-Match Strategy filled in → `obsidian write "Dating/<Name>/conversation.md" --file $TMPDIR/conversation.md`
...
1. `obsidian read "Dating/<Name>/conversation.md"` - get current state + last exchange number.
2. Ask: what did she send, what date, was it a response to something specific?
3. Build `## Exchange N` block → `obsidian append "Dating/<Name>/conversation.md" --file $TMPDIR/exchange.md`
...
1. Read vault context:
   ```
   obsidian read "Dating/<Name>/profile.md"
   obsidian read "Dating/<Name>/conversation.md"
...
7. When user says "I sent option [X]" or pastes the actual text:
   - Append a `## My Reply (YYYY-MM-DD)` block under the current exchange via `obsidian append "Dating/<Name>/conversation.md" --file $TMPDIR/reply.md`

### 4. Update profile

Add or revise information on an existing match.

1. `obsidian read "Dating/<Name>/profile.md" --map` to see current sections.
2. Determine target section from what the user shared. Most new observations go into `## Notes & Observations`.
3. For appending within a section: `obsidian patch "Dating/<Name>/profile.md" --target-type heading --target "Notes & Observations" --operation append --file $TMPDIR/note.md`
