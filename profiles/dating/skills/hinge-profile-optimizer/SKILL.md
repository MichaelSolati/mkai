...
0.5. **Pre-flight** - Load user dossier (silent; informs discovery)
1. **Audit** - Score current profile (skip if starting fresh)
2. **Discovery** - The big interview - find the real person (dossier-aware)
3. **Reality Check** - Market math, settings review
4. **Photos** - Evaluate, order, identify gaps
5. **Copy** - Write prompts and captions
5.5. **Persist findings** - Save identity-level discoveries to dossier
...
## Phase 0.5: Pre-flight - Load User Dossier
...
- Load it into working context.
- Note which sections are already populated - these map to discovery questions that can be shortened or skipped.
- See `references/dossier-integration.md` for the mapping of dossier sections to Phase 2 question areas.
- Do NOT tell the user you're doing this - just let it silently inform the conversation. You can reference it naturally ("I see you've mentioned you're in Brooklyn - anything else about your neighborhood worth putting in the profile?").

**If the dossier is absent:**
- Note to user: "I don't have a dossier on you yet - the discovery interview will cover this ground, and I can save what we learn to a dossier afterward so future sessions can pick up where we left off."
- Proceed with full Phase 2 Discovery.
...
**If a dossier was loaded in Phase 0.5:** Before asking each question area, check whether the dossier already answers it. If yes, frame as confirmation/extension rather than a fresh question: *"Your profile mentions you're a software engineer in Brooklyn - any angle on the work you'd want in the profile, or should we skip the work section?"* If the dossier gives only partial coverage, ask what's missing. See `references/dossier-integration.md` for section mappings.
...
## Phase 5.5: Persist Findings to Dossier

After delivering the copy doc, offer to save identity-level findings back to the user dossier.

**What qualifies:**
- Anything learned in Discovery that reveals stable facts about the user (job, neighborhood, interests, lifestyle)
- Strong opinions and personality markers that emerged
- What they're looking for and any dealbreakers surfaced in Phase 3
- Date repertoire (go-to spots, what a good first date looks like to them)

**How to offer:**
> "We covered a lot about you in the discovery phase. Want me to save the key findings to your dossier so next time we - or the match-tracker - picks up context automatically without re-asking?"
