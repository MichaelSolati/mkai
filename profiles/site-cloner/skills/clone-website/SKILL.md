...
1. **Browser automation is required.** This skill uses the `chrome-devtools` skill for all page inspection, screenshot capture, DOM traversal, and JavaScript evaluation. Ensure Chrome is running with remote debugging enabled (the chrome-devtools skill handles connection). This skill cannot work without browser automation.
2. Parse `$ARGUMENTS` as one or more URLs. Normalize and validate each URL; if any are invalid, ask the user to correct them before proceeding. For each valid URL, navigate to it using the chrome-devtools skill and verify it loads correctly.
...
- Click each tab/button via the chrome-devtools skill
...
The spec file is not optional. It is not a nice-to-have. If you dispatch a builder without first writing a spec file, you are shipping incomplete instructions based on whatever you can remember from a chrome-devtools session, and the builder will guess to fill gaps.
...
Navigate to the target URL using the chrome-devtools skill.
...
**Scroll sweep:** Scroll the page slowly from top to bottom using the chrome-devtools skill. At each section, pause and observe:
...
**Responsive sweep:** Test at 3 viewport widths using the chrome-devtools skill to resize the viewport:
- Desktop: 1440px
- Tablet: 768px
- Mobile: 390px
...
Use the chrome-devtools skill to enumerate all assets on the page:

```javascript
// Run this via chrome-devtools skill to discover all assets
JSON.stringify({
...
For each section, use the chrome-devtools skill to extract everything:
...
```javascript
// Per-component extraction - run via chrome-devtools skill
...
// Then trigger the state change (scroll, click, hover via chrome-devtools skill)
...
4. For each discrepancy found:
   ...
   - If the spec was wrong: re-extract using the chrome-devtools skill, update the spec, fix the component
