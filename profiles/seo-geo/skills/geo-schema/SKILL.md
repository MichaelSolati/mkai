...
1. Fetch the target page HTML using `curl` (see Step 1 below)
...
**IMPORTANT:** WebFetch converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. Use `curl` to fetch the raw HTML instead:
```bash
curl -sL "<url>" -o "$TMPDIR/page.html"
```
Then read the downloaded HTML file to extract structured data. Look for `<script type="application/ld+json">` blocks and parse them as JSON.
