...
Class `CrossrefClient` exposes `doi_lookup_with_title_check(doi, expected_title)` and `title_search(title, year=None)`. Both return `dict | None` (the dict being either the `message` for DOI, or one item from `message.items` for title search). Both raise `CrossrefUnavailable` on degradation per the table above.

## Cross-references

- Mirror template: `references/semantic_scholar_api_protocol.md`
- Sibling protocol: `references/openalex_api_protocol.md`
