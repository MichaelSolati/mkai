...
Before emitting any final converted artifact (LaTeX / DOCX / PDF), scan the input markdown for unresolved citation-provenance markers per the Cite-Time Provenance Finalizer protocol. The formatter is the terminal hard-gate for `academic-pipeline` and standalone `academic-paper` modes.
...
External motivation for rule 3: Zhao et al. arXiv:2605.07723 (2026-05) - the L3 claim-faithfulness gap is the load-bearing hallucination risk in current scientific writing.
