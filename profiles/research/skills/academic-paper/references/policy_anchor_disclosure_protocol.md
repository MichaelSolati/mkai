...
2. **Selector**: `--policy-anchor=<a>` where `a ∈ {prisma-trAIce, icmje, nature, ieee}`. **Selector-mutually-exclusive by default** - supplying both `--policy-anchor` and `--venue` triggers the §5 conflict-resolution flow, which permits exactly one consistent pair (any Nature Portfolio venue + `--policy-anchor=nature`, sharing the canonical Nature policy source) and rejects every other combination with explicit error. See §5 for the full enumeration.
...
**Dedup with v3.2 Nature venue path:** the Nature substantive policy text is shared between the v3.2 Nature venue renderer in `venue_disclosure_policies.md` and this Nature anchor renderer. **G4 invariant**: edits to Nature-specific policy quotes must be consistent across both consumers.
...
Silent precedence is **forbidden** (Decision Doc §4.4 #7 + concern #7 resolution). The renderer must surface the conflict to the user and require an explicit selector choice.
...
- Decision Doc: AI Disclosure Schema Decision Doc
- Implementation spec: AI Disclosure Implementation Spec
