...
**Provenance**: cells below carry verbatim policy quotes lifted from the discovery doc §4.3-4.6 source-of-truth. Each anchor section records its snapshot id + truncated sha256 so the lint validator can confirm provenance integrity. Live URLs may drift after capture; the wayback snapshot is canonical.

**Lint contract**: enforces 4-anchor coverage, 16-field-per-anchor canonical order, source_strength enum membership, and verbatim-quote presence for mandate/recommend/conditional cells.

**Nature ↔ v3.2 venue de-dup**: the Nature anchor below shares its substantive policy content with the v3.2 Nature venue renderer in `venue_disclosure_policies.md`. Both files cross-reference a canonical source pointer so a future single-source-of-truth refactor can byte-compare imported substrings without breaking either consumer.
...
- The shared Nature policy source MUST live outside any anchor-specific or venue-specific file so neither consumer can drift unilaterally.
...
**Source-of-truth pointer:** the canonical substantive policy text lives in a shared Nature policy source; the v3.2 venue renderer (`venue_disclosure_policies.md` Nature entry) shares this source. **De-dup invariant:** edits to Nature-specific policy quotes must go through the canonical source first; both consumers re-cite from there.
...
- Decision Doc (parent): AI Disclosure Schema Decision Doc
- Implementation spec (parent): AI Disclosure Implementation Spec
- Discovery doc (verbatim quote source-of-truth): AI Disclosure Schema Discovery §4.3-4.6
...
- Shared Nature policy source (canonical pointer, forthcoming)
