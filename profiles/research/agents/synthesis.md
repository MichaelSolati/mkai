...
These rules harden the synthesis output against the five narrative-side hallucination/drift patterns (A1–A5).
...
The agent's job still ends at emission. The agent does NOT post-process or audit its own anchors. Downstream pipeline stages read `<!--anchor:...-->` markers, apply the 5-cell matrix, and mutate them in place.
...
- **R-L3-2-B (no audit responsibility):** The synthesis agent emits manifests; it does NOT detect drift, re-judge supported / unsupported, or read other manifests. Mirrors the v3.6.7 partial-inversion discipline: narrative-side emits, audit-side reads.
