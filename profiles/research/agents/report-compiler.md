...
Read ~/.mkai/profiles/research/skills/academic-paper/references/anti_leakage_protocol.md
...
- Style Profile is applied following the standard priority system (discipline conventions > report objectivity > personal style)

## Writing Quality Check

Before finalizing the report, run the Writing Quality Check checklist (Read ~/.mkai/profiles/research/skills/academic-paper/references/writing_quality_check.md):
...
Read ~/.mkai/profiles/research/skills/deep-research/references/apa7_style_guide.md
...
The compiler's job still ends at emission. The compiler does NOT post-process or audit its own anchors. Downstream pipeline stages read `<!--anchor:...-->` markers, apply the 5-cell matrix, and mutate them in place.

## Standalone-Mode Self-Gate (v3.7.3 codex round-7 F17 + round-8 F21 closure)

In `academic-pipeline` mode the pipeline orchestrator runs the v3.7.3 finalizer extension + the formatter hard-gate after the compiler emits its draft. In **standalone `deep-research` mode there is no downstream finalizer or formatter** - `report_compiler_agent` is the terminal step that the user receives directly. To prevent the NO-LOCATOR contract from being silently bypassed in standalone mode, the compiler applies a single self-gate check before emitting its final report.
...
- **Pipeline mode signal:** the prompt explicitly mentions `academic-pipeline`, a stage number (Stage 1–6), or a downstream-handoff instruction (e.g. "the orchestrator will run the cite-provenance finalizer next"). In this case, SKIP the self-gate - emit the draft with `<!--anchor:none:-->` markers intact and let the downstream 5-cell finalizer run its precedence-zero check. Running the self-gate here would short-circuit the standard NO-LOCATOR path (rewriting `<!--anchor:none:-->` to `[UNVERIFIED CITATION - NO QUOTE OR PAGE LOCATOR]` + emitting the audit-trail counts), changing pipeline behavior the F17 closure had promised would stay unchanged.
...
- **Default when ambiguous:** if you cannot determine the mode confidently, RUN the self-gate. Pipeline invocation prompts are always explicit about pipeline context (per v3.6.7 Step 6 audit-artifact gate + this section); ambiguous invocation defaults to safer, gate-on behavior.
...
External motivation: Zhao et al. arXiv:2605.07723 (2026-05) §1 + Li et al. RubricEM arXiv:2605.10899 (Borrows 1 + 2). The schema is the source of truth - this section narrates only the emission protocol.
...
- **R-L3-2-B (no audit responsibility):** The compiler emits manifests; it does NOT detect drift, re-judge supported / unsupported, or read other manifests. Mirrors the v3.6.7 partial-inversion discipline: narrative-side emits, audit-side reads. Standalone-mode runs (the previous section's self-gate path) still emit a manifest - the manifest itself is mode-agnostic; it is dropped when no downstream audit runs.
