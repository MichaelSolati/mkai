...
import { discoverProfiles, findProfile, formatItemCount } from "../src/profile.js";
import { readState, isActive } from "../src/state.js";
import { activateProfile } from "../src/activate.js";
import { deactivateProfile, deactivateAll } from "../src/deactivate.js";
import { isHealthySymlink } from "../src/symlink.js";
...
program
  .name("cpx")
  .description("Claude Profile Loader - manage groups of agents, skills, and commands")
  .version("0.1.0")
  .action(async () => {
    await runTui();
  });
...
program
  ...
  .action(async (opts) => {
    ...
    for (const prof of profiles) {
      ...
      console.log(`  ${chalk.bold(prof.name)}${status} - ${formatItemCount(prof)}`);
...
program
  ...
  .action(async (opts) => {
    ...
    const activations = opts.all
      ? state.activations
      : state.activations.filter((a) => a.target === "global" || a.projectPath === cwd);
    ...
    for (const activation of activations) {
      const targetLabel = activation.target === "project"
        ? `project: ${activation.projectPath}`
        : "global";
      console.log(`  ${chalk.bold(activation.profile)} (${targetLabel}) - ${activation.links.length} links`);
  ...
  .action(async (profileNames: string[], opts) => {
    ...
    for (const name of profileNames) {
      ...
      if (result.success) {
        ...
        console.log(chalk.green(`${prefix}Activated "${name}" - ${result.linksCreated} links created.`));
      } else {
        console.error(chalk.red(`Failed to activate "${name}": ${result.errors.join("; ")}`));
      }
    }
  });
...
program
  ...
  .action(async (profileNames: string[], opts) => {
    if (opts.all) {
      ...
      const prefix = opts.dryRun ? "[dry-run] " : "";
      console.log(chalk.green(`${prefix}Deactivated all - ${result.linksRemoved} links removed.`));
      return;
    }
    ...
    for (const name of profileNames) {
      ...
      const result = await deactivateProfile(name, target, { dryRun: opts.dryRun });
      const prefix = opts.dryRun ? "[dry-run] " : "";
      console.log(chalk.green(`${prefix}Deactivated "${name}" - ${result.linksRemoved} links removed.`));
    }
  });
  ...
  .action(async () => {
    ...
    for (const activation of state.activations) {
      for (const link of activation.links) {
        ...
        if (!healthy) {
          console.log(chalk.red(`  broken: ${activation.profile}/${link.name} → ${link.destination}`));
          broken++;
        }
      }
    }
...
program
  ...
  .action(async (name: string) => {
    const { profilesDir } = await import("../src/config.js");
